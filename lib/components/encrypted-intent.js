...
exports.EncryptedIntent = void 0;
const encryption_1 = require("./encryption");
const __1 = require("..");
const matrix_bot_sdk_1 = require("@vector-im/matrix-bot-sdk");
const intent_1 = require("./intent");
const log = new __1.Logger("EncryptedIntent");
/**
 * Implements some special handling on top of Intent to handle encrypted rooms.
 */
class EncryptedIntent extends intent_1.Intent {
    encryptionOpts;
    encryptedRooms = new Map();
    encryptionReadyPromise;
    // A client that talks directly to the homeserver, bypassing pantalaimon.
    encryptionHsClient;
    constructor(botSdkIntent, botClient, intentOpts, encryptionOpts) {
        super(botSdkIntent, botClient, intentOpts);
        this.encryptionOpts = encryptionOpts;
        // We still need a direct client to the homeserver in some cases, so clone
        // the existing one.
        this.encryptionHsClient = new matrix_bot_sdk_1.MatrixClient(this.encryptionOpts.originalHomeserverUrl, this.botSdkIntent.underlyingClient.accessToken);
    ...
    async loginForEncryptedClient() {
        ...
        const res = await this.botSdkIntent.underlyingClient.doRequest("POST", "/_matrix/client/v3/login", undefined, {
            type: encryption_1.APPSERVICE_LOGIN_TYPE,
            identifier: {
                type: "m.id.user",
                user: userId,
            }
    ...
    async getEncryptedSession() {
        ...
        if (session) {
            ...
            try {
                const tempClient = new matrix_bot_sdk_1.MatrixClient(this.botSdkIntent.underlyingClient.homeserverUrl, session.accessToken);
        ...
        underlyingClient.accessToken = session.accessToken;
    ...
    async isRoomEncrypted(roomId) {
        ...
        try {
            ...
            if (ex instanceof matrix_bot_sdk_1.MatrixError && ex.statusCode === 404) {
                this.encryptedRooms.set(roomId, false);
                return false;
            }
            throw ex;
...
exports.EncryptedIntent = EncryptedIntent;
//# sourceMappingURL=encrypted-intent.js.map